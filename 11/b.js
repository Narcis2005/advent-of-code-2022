...
readModuleFile('./input.txt', async function (err, input) { 
    ...
    for (let monkey in monkeysObject) {
        modulo*=monkeysObject[monkey]["test"];
    }
        ...
        for(let monkey in monkeysObject) {
            ...
            for(let j = 0; j<length; j++) {
                ...
                if(currentNumber % monkeysObject[monkey]["test"] === 0) {
                    monkeysObject[monkeysObject[monkey]["ifTrue"]]["startingItems"].push(currentNumber%modulo);
                }
                else monkeysObject[monkeysObject[monkey]["ifFalse"]]["startingItems"].push(currentNumber%modulo);
