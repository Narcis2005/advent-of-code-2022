...
readModuleFile('./input.txt', function (err, input) {  
    ...
    pairsInArray.forEach(pair => {
        if(parseInt(pair[0].split("-")[0]) >= parseInt(pair[1].split("-")[0]) && parseInt(pair[0].split("-")[1]) <= parseInt(pair[1].split("-")[1])) {
            numberOfContainedPairs++;
        }
        else if(parseInt(pair[0].split("-")[0]) <= parseInt(pair[1].split("-")[0]) && parseInt(pair[0].split("-")[1]) >= parseInt(pair[1].split("-")[1])) {            
            numberOfContainedPairs++;
        }
    })
